    ...
    try {
      ...
      while (!_abortRequested) {
        ...
        if (mode == ValidationMode.velocity) {
          ...
          // Convert user velocity units to RPM for the controller.
          final rpmSetpoint =
              setpoint / mechanismConfig.velocityConversionFactor;
          device.control.setVelocity(rpmSetpoint);
        } else {
          setpoint = inHoldPhase ? params.positionSetpoint : params.positionSetpoint;
          // Convert user position units to rotations for the controller.
          final rotSetpoint =
